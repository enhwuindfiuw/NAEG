...
tf.flags.DEFINE_string(
    'output_dir', 'alpha/NI-FGSM/epsilon18000_iter300/incepv4', 'Output directory with images.')

tf.flags.DEFINE_float(
    'max_epsilon', 18000.0, 'Maximum size of adversarial perturbation.')
...
def graph(x, y, i, x_max, x_min, grad, m, v):
    eps = 2.0 * FLAGS.max_epsilon / 255.0
    num_iter = FLAGS.num_iter
    alpha = eps / num_iter  # 初始学习率
    beta1 = 0.9  # 动量系数 (β1)
    beta2 = 0.999  # 二阶矩衰减率 (β2)
    epsilon = 1e-8  # 数值稳定项

    # 初始化 Nadam 的动量变量（第一次迭代时创建）
    if m is None:
        m = tf.zeros_like(x)
    if v is None:
        v = tf.zeros_like(x)

    # 模型前向计算（以 inception_v4 为例）
    with slim.arg_scope(inception_v4.inception_v4_arg_scope()):
        logits_v4, end_points_v4 = inception_v4.inception_v4(
    ...
    # 计算损失（主 logits + 辅助 logits）
    logits = logits_v4
    auxlogits = end_points_v4['AuxLogits']
    cross_entropy = tf.losses.softmax_cross_entropy(one_hot, logits, weights=1.0)
    cross_entropy += tf.losses.softmax_cross_entropy(one_hot, auxlogits, weights=0.4)

    # 计算当前梯度并归一化
    current_grad = tf.gradients(cross_entropy, x)[0]
    current_grad = current_grad / tf.reduce_mean(tf.abs(current_grad), [1, 2, 3], keep_dims=True)

    # Nadam 更新规则
    t = tf.cast(i, tf.float32) + 1.0  # 迭代次数校正
    m_t = beta1 * m + (1 - beta1) * current_grad  # 一阶矩（动量）
    v_t = beta2 * v + (1 - beta2) * tf.square(current_grad)  # 二阶矩

    # 偏差校正
    m_hat = m_t / (1 - tf.pow(beta1, t))
    v_hat = v_t / (1 - tf.pow(beta2, t))

    # Nesterov 加速：用未来动量方向近似
    m_nesterov = beta1 * m_hat + (1 - beta1) * current_grad/(1 - tf.pow(beta1, t))

    # 计算自适应学习率和更新噪声
    adaptive_lr = alpha / (tf.sqrt(v_hat) + epsilon)
    noise = m_nesterov  # 直接使用 Nesterov 动量作为噪声
    x = x - adaptive_lr * noise

    # 像素值裁剪
    x = tf.clip_by_value(x, x_min, x_max)
    i = tf.add(i, 1)

    return x, y, i, x_max, x_min, noise, m_t, v_t



def stop(x, y, i, x_max, x_min, grad, m, v):
    num_iter = FLAGS.num_iter
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        m = tf.zeros(shape=batch_shape)
        v = tf.zeros(shape=batch_shape)
        x_adv, _, _, _, _, _, _, _ = tf.while_loop(stop, graph, [x_input, y, i, x_max, x_min, grad, m, v])
