...
def graph(x, y, i, x_max, x_min, grad, accum):
    eps = 2.0 * FLAGS.max_epsilon / 255.0
    num_iter = FLAGS.num_iter
    alpha = eps / num_iter  # 初始学习率
    decay_rate = 0.9        # RMSProp 衰减率
    epsilon = 1e-8          # 防止除零

    # 初始化 RMSProp 的梯度平方累积量（第一次迭代时创建）
    if accum is None:
        accum = tf.zeros_like(x)
    ...
    # RMSProp 更新规则
    new_accum = decay_rate * accum + (1 - decay_rate) * tf.square(current_grad)
    adaptive_lr = alpha / (tf.sqrt(new_accum) + epsilon)

    # 更新噪声（结合历史梯度方向）
    noise = grad + current_grad  # 取消动量，直接累加梯度
    x = x - adaptive_lr * tf.sign(noise)  # 自适应步长更新
    x = tf.clip_by_value(x, x_min, x_max)

    # 迭代次数更新
    i = tf.add(i, 1)
    return x, y, i, x_max, x_min, noise, new_accum
